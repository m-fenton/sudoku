...
func main() {
	if len(os.Args) != 10 {
		...
		if len(os.Args) != 1 {
			if len(os.Args) == 10 || len(os.Args[1]) == 89 {
				unstringed := ArgsValid(os.Args)
				board := parseInput(unstringed)
				if backtrack(&board) {
					printBoard(board)
				} else {
					fmt.Printf("Error\n")
...
func ArgsValid(args []string) string {
	validString := ""
	for i := 1; i < len(args); i++ {
		for _, nb := range args[i] {
			if nb != ' ' {
				validString += string(nb)
			}
		}
	}
	return validString
